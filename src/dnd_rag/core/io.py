...
class Chunk:
    ...
    Текстовый чанк для эмбеддинга и поиска.
    
    Attributes / Атрибуты:
        text: Chunk text content / Текстовое содержимое чанка
        chapter: Top-level chapter name / Название главы верхнего уровня
        section: Section name / Название секции
        page_start: First page number / Номер первой страницы
        page_end: Last page number / Номер последней страницы
        tokens: Token count for this chunk / Количество токенов в чанке
    """
    text: str
...
# Pattern for code fence markers: ```
# Шаблон для маркеров блоков кода: ```
...
def _split_into_blocks(text: str) -> List[str]:
    ...
    Разбивает текст на логические блоки: параграфы, таблицы, блоки кода, списки.
    
    This function preserves the structure of special Markdown elements.
    Code blocks and tables are kept atomic (not split across chunks).
    
    Эта функция сохраняет структуру специальных элементов Markdown.
    Блоки кода и таблицы сохраняются атомарными (не разбиваются на чанки).

    Simple heuristics / Простые эвристики:
    - Fenced code blocks (```) are kept together / Блоки кода в тройных кавычках сохраняются вместе
    - Tables (lines with multiple '|') are kept together / Таблицы (строки с несколькими '|') сохраняются вместе
    - Paragraphs are split on blank lines / Параграфы разделяются пустыми строками
    
    Parameters / Параметры:
        text: Section text to split / Текст секции для разбиения
        
    Returns / Возвращает:
        List of text blocks / Список текстовых блоков
    """
    lines = text.splitlines()
    blocks: List[str] = []
    cur: List[str] = []
...
def chunk_sections(
    ...
    """
    Split sections into chunks suitable for embedding, with overlap between chunks.
    Разбивает секции на чанки, подходящие для эмбеддинга, с перекрытием между чанками.
    
    This function creates text chunks that fit within a token limit while preserving
    context through overlapping tokens. Each chunk includes the section header for context.
    
    Эта функция создаёт текстовые чанки, укладывающиеся в лимит токенов, сохраняя
    контекст через перекрывающиеся токены. Каждый чанк включает заголовок секции для контекста.
    
    Strategy / Стратегия:
    1. Add blocks to chunk until token limit reached / Добавляем блоки в чанк до достижения лимита токенов
    2. When limit reached, save chunk and start new one / При достижении лимита сохраняем чанк и начинаем новый
    3. Include overlap tokens from previous chunk / Включаем перекрывающиеся токены из предыдущего чанка
    4. Blocks larger than max_tokens are stored as standalone chunks
       Блоки больше max_tokens сохраняются как отдельные чанки
    
    Parameters / Параметры:
        sections: Iterable of Section objects / Итерируемая коллекция объектов Section
        max_tokens: Maximum tokens per chunk (default 800) / Максимум токенов на чанк (по умолчанию 800)
        overlap: Token overlap between consecutive chunks (default 120)
                 Перекрытие токенов между последовательными чанками (по умолчанию 120)
    
    Returns / Возвращает:
        List of Chunk objects ready for embedding / Список объектов Chunk, готовых для эмбеддинга
    """
    chunks: List[Chunk] = []
    for sec in sections:
        # Create section header with appropriate number of '#' symbols
        # Создаём заголовок секции с соответствующим количеством символов '#'
...
def _chapter_from_path(path: Sequence[Tuple[int, str]]) -> Optional[str]:
    """
    Extract chapter name from heading path (breadcrumb trail).
    Извлекает название главы из пути заголовков (цепочки навигации).
    
    The chapter is defined as the highest-level (smallest number) heading in the path.
    Глава определяется как заголовок наивысшего уровня (наименьшее число) в пути.
    
    Example / Пример:
    - Path: [(1, "Chapter 1"), (2, "Section A"), (3, "Subsection i")]
    - Returns: "Chapter 1" (level 1 is highest / уровень 1 наивысший)
    
    Parameters / Параметры:
        path: Sequence of (level, title) tuples / Последовательность кортежей (уровень, заголовок)
        
    Returns / Возвращает:
        Chapter title or None / Название главы или None
    """
    if not path:
        return None
    # Chapter: the highest-level heading encountered so far (level == min)
    # Глава: заголовок наивысшего уровня из встреченных (уровень == минимум)
